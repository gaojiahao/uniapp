<template>
  <el-container>
    ...
    <div class="tableContent">
      ...
      >
        <el-table-column prop="shieldName" label="展厅名称"></el-table-column>
        <el-table-column prop="shieldNumber" label="展厅编号"></el-table-column>
        <el-table-column
          prop="createdOn"
          label="屏蔽时间"
          sortable
    ...
    <!-- 选择屏蔽展厅dialog -->
    <el-dialog title="选择展厅" :visible.sync="addHallDialog" destroy-on-close width="30%">
      <el-form
        ref="addVersionForm"
        label-width="100px"
        :model="FormData"
      >
        ...
        <center>
          <template>
            ...
            <el-button type="danger" @click="addHallDialog = false">取 消</el-button>
          </template>
        </center>
      </el-form>
    </el-dialog>
  ...
  methods: {
    ...
    async getOrgCompanyList () {
      ...
      if (res.data.result.code === 200) {
        this.hallList = res.data.result.item.filter(val => (val && val.companyNumber !== this.$store.state.userInfo.commparnyList[0].companyNumber))
      } else {
    ...
    async subProcessingLog () {
      ...
      const res = await this.$http.post('/api/InsertableExhibitionShield', { shieldNumber: fd.companyNumber, shieldName: fd.companyName })
      if (res.data.result.code === 200) {
