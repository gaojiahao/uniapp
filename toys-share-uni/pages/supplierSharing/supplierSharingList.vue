<template>
	<view class="supplier_sharing_list">
		<!-- pc端 -->
		<xHead active="supplierSharingList" :isMobile="isMobile"></xHead>
	...
	methods:{
		...
		async onReachBottom() {
			...
			if(me.isMobile){
				...
				if (res.result.code === 200) {
					if(res.result.item.threeDProduct.newList.length){
						for(var i=0;i<res.result.item.threeDProduct.newList.length;i++){
							me.proudctList.push(res.result.item.threeDProduct.newList[i]);
						}
						me.page++;
						me.status = 'loading';
						me.$loading.hide();
