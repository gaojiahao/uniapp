<template>
	<div class="magnify">
        ...
        >
          ...
          >
            <pic-zoom
              :url="require('@/static/images/imgError.png')"
              :scale="3"
            ></pic-zoom>
          </div>
          <div
        ...
        >
          <div
            ...
            style="width: 524px;height: 393px;"
          >
            <img :src="require('@/static/images/imgError.png')" />
          </div>
          <div
            slot="error"
            class="image-slot"
          ...
          >
            <img :src="require('@/static/images/imgError.png')" />
				...
				<!-- 产品视频 -->
				<video v-else-if="middleImg.type === 'video'" id="example_video" :width="middleImgWidth"
					:height="middleImgHeight" class="video-js vjs-default-skin bigVideo" controls poster="">
			...
			<div class="carousel" v-if="imageUrls.length || videoAddress">
				...
				<div class="show_box">
					<ul class="picture_container" ref="middlePicture">
						<li class="picture_item" v-if="videoAddress" @mouseover="tabPicture({ type: 'video' })">
							<div class="imgBox">
								<video style="border: 1px solid #dcdfe6" id="example_video" :width="thumbnailWidth"
									:height="thumbnailHeight" class="video-js vjs-default-skin minVideo" poster="">
									<source :src="videoAddress" type="video/mp4" />
								</video>
							</div>
						</li>
						<li class="picture_item" @mouseover="tabPicture({ type: 'img', url: item })"
							v-for="(item, index) in pictureList" :key="index">
							<div class="imgBox">
		...
		<div class="right_contanier" v-show="isBig">
			<image :src="middleImg.url" ref="bigImg" class="big_img" alt="" /></image>
			<image :src="middleImg.url" ref="bigImg" class="big_img">
				
			</image>
		</div>
		<!-- 3d弹窗 -->
		<u-modal v-model="dialogVisibleThreeD" :width="isZoom?'100%':'80%'" :show-confirm-button="false"
			:mask-close-able="true" :show-title="false">
			<view class="slot-content">
				...
				<view :class="[isZoom ? 'bg_panel' : 'sm_panel']">
					<iframe :src="threeDimensional" id="map" scrolling="no" frameborder="0" class="sm_iframe"></iframe>
				</view>
			</view>
		</u-modal>
	</div>
...
<script>
	...
	export default {
		...
		methods: {
			rerunImg(img) {
				if (img) return img;
				else return require("@/static/images/imgError.png");
			},
			// 打开3D效果
			open3D(value) {
			...
			//关闭pc3d弹窗
			close_pc_modal() {
				this.dialogVisibleThreeD = false;
			},
		}
	};
</script>
...
<style scoped lang="less">
	...
	.left_contaner .middle_img {
		...
		.threeIcon {
			...
			width: 60px;
			height: 60px;
			background: url("~@/static/images/orange3D.png") no-repeat center;
